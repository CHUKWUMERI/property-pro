...
class RentProp(scrapy.Spider):
    ...
    start_urls = [
        f"https://www.propertypro.ng/{prop_type}search=&auto=&type=&bedroom=&min_price=&max_price=&page={page_numb}"
        for prop_type in [
            "property-for-sale?",
            "property-for-rent?",
    ...
    def parse(self, response):
        ...
        for prop in response.css("div.single-room-text"):
            ...
            price = prop.css(
                "div.n50.featured-text > h3 > span:nth-child(2)::text"
            ).getall()

            # Bedrooms, baths and toilets
            bedrooms = prop.css(
                "div.single-room-text > div.fur-areea > span:nth-child(1)::text"
            ).get()
            baths = prop.css(
                "div.single-room-text > div.fur-areea > span:nth-child(2)::text"
            ).get()
            toilets = prop.css(
                "div.single-room-text > div.fur-areea > span:nth-child(3)::text"
            ).get()
            ...
            currency, price, phone_number, date_added = (
                "".join(currency),
                "".join(price),
                "".join(phone_number).strip("\n"),
                "".join(date_added).strip("\n"),
            )

            # This will follow the read more and extract the data stored there
            read_more_url = prop.css(
                "div.result-list-details > p > a::attr('href')"
            ).get()
            ...
            more_details = scrapy.Request(
                more_det_url, callback=self.get_details, cb_kwargs={"content": content}
            )
            yield more_details

    def get_details(self, response, content):
        # This function will get the details from each read more
        more_details = response.css(
            "#tabs-1 > div > div.key-features-area > div.description-area"
        )
        ...
        for det in more_details:
            ...
            content[
                "More Details"
            ] = details  # Assigns the more details page per read more to each content
            yield content
